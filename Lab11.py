...
for r in range(1,int(sheet.max_row)+1):
    student_list.append(sheet.cell(row=r, column=1).value)
    tuition_code_list.append(sheet.cell(row=r, column=2).value)
    sports_code_list.append(sheet.cell(row=r, column=3).value)
    lunch_code_list.append(sheet.cell(row=r, column=4).value)
    status_code_list.append(sheet.cell(row=r, column=5).value)
    how_to_pay_list.append(sheet.cell(row=r, column=6).value)
...
for i in range(len(tuition_code_list)):
    if tuition_code_list[i] ==int("1"):
        tuition_list=tuition_list+[8000]
    elif tuition_code_list[i] ==int("0"):
...
for r in range(len(total_amount_list)): 
    sheet.cell(row=r+1, column=1).value = student_list[i] 
    sheet.cell(row=r+1, column=2).value = tuition_code_list[i] 
    sheet.cell(row=r+1, column=3).value = sports_code_list[i] 
    sheet.cell(row=r+1, column=4).value = lunch_code_list[i] 
    sheet.cell(row=r+1, column=5).value = status_code_list[i]
    sheet.cell(row=r+1, column=6).value = how_to_pay_list[i]
    sheet.cell(row=r+1, column=7).value = total_amount_list[i]
